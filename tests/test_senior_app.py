...
@pytest.mark.asyncio
async def test_company_name_autocomplete(api):
    """
    1. 회사명 자동완성
    회사명의 일부만 들어가도 검색이 되어야 합니다.
    header의 x-wanted-language 언어값에 따라 해당 언어로 출력되어야 합니다.
    """
    resp = await api.get("/search?query=링크", headers=[("x-wanted-language", "ko")])
...
@pytest.mark.asyncio
async def test_company_search(api):
    """
    2. 회사 이름으로 회사 검색
    header의 x-wanted-language 언어값에 따라 해당 언어로 출력되어야 합니다.
    """
    ...
    assert resp.status_code == 200
    assert company["company_name"] == "원티드랩"
...
@pytest.mark.asyncio
async def test_new_company(api):
    """
    3.  새로운 회사 추가
    ...
    company = json.loads(resp.content.decode("utf-8"))
    assert company["company_name"] == "LINE FRESH"
    for tag_name in ["tag_1", "tag_8", "tag_15",]:
        assert tag_name in company["tags"]


@pytest.mark.asyncio
async def test_search_tag_name(api):
    """
    4.  태그명으로 회사 검색
    ...
    """
    resp = await api.get("/tags?query=タグ_22", headers=[("x-wanted-language", "ko")])
    searched_companies = json.loads(resp.content.decode("utf-8"))
...
async def test_new_tag(api):
    ...
    for tag_name in ["tag_4", "tag_50"]:
        assert tag_name in company["tags"]


@pytest.mark.asyncio
async def test_delete_tag(api):
    """
    6.  회사 태그 정보 삭제
    저장 완료후 header의 x-wanted-language 언어값에 따라 해당 언어로 출력되어야 합니다.
    """
    ...
    company = json.loads(resp.content.decode("utf-8"))
    assert company["company_name"] == "Wantedlab"
    assert "태그_16" not in company["tags"]
